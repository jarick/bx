...
## Для работы необходимо php 5.4 либо более новая версия, memcache(для хранения кеша в случае отсуствия сайт не будет работать с кешем), mysql или sqlite. 

##### Стандарт кодирования.

Framework полностью построен на стандартнах psr. За исключением правила использования табуляции(используются табы вместо пробелов).

##### Структура.

Код располагается в папке src и разбит на независимые компоненты. Структура компонента как правило разбивается на 4 бизнес слоя:
...
Свои фильтры и валидаторы для полей можно задать с помощью функции $this->rule()->custom
Пример:
```php	
...
```
Как видно из примера для того чтобы поменять значение введеного поля нужно изменить переменную $value. Чтобы сгенерировать ошибку нужно вернуть строку с ошибкой. Если валидация прошла успешно, то можно ничего не возвращать или вернуть null. Более подродно валидация описана в глава Валидации.

Entity инкапсулирует в себе значение полей, названия полей, валидаторы и ошибки валидации. Чтобы задать значение для сущности нужно передать массив со значениями в функции setData(). В функции безопасно можно вставлять значения из Request параметров.

Чтобы свалидировать значение нужно выполнить функцию checkFields. Для получения ошибок нужно вызвать функцию getErrors.

* Слой хранения данных(Store). В этом слою описана логика работы со сущностями(Entity) для различных баз данных. К пример сессию пользователя мы можем хранить в сессии PHP, в базе данных, или noSql хранилищах. В этом случае бизнесс логике не обязательно знать какое-именно хранилище использовать. Бизнесс-логика будет работать с единным интерфейсом для всех хралиш и с одной и той же сущностью.
...
* Слой управления данными(Manager). Главный слой в которым находится вся логика по работе с данными. Как правило вызов менеджеров спрятан в трейты, которые хранят их в ioc хранилище(см. класс BX\Base\DI). Что позволяет не меняя код переопределить стандартные менеджеры на свой класс. Пример:
```php
...
```
В данном примере мы переопределили стандартный менеджер кеша на свой Mock класс чтобы проверить корректную работу функции clearCache. Затем мы очистили контейнер, чтобы framework работал с менеджером кеша по умолчанию.
...
ORM фреймворка написан с помощью паттерна UnitOfWork, гарантирует защиту от Race condition . Пример изменения данных:
```php
	/**
	 * Clear old captcha
	 * @param integer $day
...
Чтобы осуществить фильтрацию нужно вызвать функцию finder(см. скласс BX\DB\Filter\SqlBuilder).

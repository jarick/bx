...
class SqlBuilder
{
	 ...
	 */
	public function __construct(\BX\DB\ITable $entity)
	{
		$this->entity = $entity;
	}
	/**
	...
	private function getFields()
	{
		if ($this->fields_cache === null){
			...
			foreach ($this->entity->getColumns() as $key => $column){
				...
				$this->fields_cache[$key] = $column->getColumn();
			}
		}
	...
	private function getFilterRule()
	{
		...
		foreach ($this->entity->getColumns() as $key => $column){
			$key = $this->string()->toUpper($key);
			$return[$key] = $column->getFilterRule();
		}
		return $return;
	}
	/**
	...
	{
		$block = new LogicBlock($this,$this->getFilterRule());
	...
	public function all()
	{
		return $this->convertFromArray($this->asArray()->getData());
	}
	/**
	 * Find all rows
	 * @return DBResult
